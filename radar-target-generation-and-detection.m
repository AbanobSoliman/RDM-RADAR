...
for i=2:length(t)         
    ...
    %Receiver Signal
    Mix(i) = cos(2*pi*(2*slope*r_t(i)*t(i)/c+2*fc*v0*t(i)/c));  %Tx(i).*Rx(i)
    
end

Mix = Mix + randn(size(t));
...
noise_level = 0;  %zeros(Nr-(Gr+Tr+1),Nd-(Gd+Td+1));
